...
Функция ПолучитьОбновления() Экспорт
	...
	Для Каждого Обновление Из ОписаниеОбновлений.result Цикл
		...
		ИначеЕсли Обновление.Свойство("callback_query") Тогда
			...
			CallbackQuery = Обновление.callback_query;
			
			Если CallbackQuery.Свойство("from") Тогда
				СохраненноеВходящееСообщение.Отправитель = ПолучитьОтправителя(CallbackQuery.from);
			КонецЕсли;
			
			Если CallbackQuery.Свойство("message") И CallbackQuery.message.Свойство("chat") Тогда
				СохраненноеВходящееСообщение.Чат = ПолучитьЧат(CallbackQuery.message.chat);
			КонецЕсли;
...
// Отправить сообщение и получить его ID.
// 
// Параметры:
//  Получатель - Число, СправочникСсылка.УчетныеЗаписиТелеграм - Получатель сообщения
//  Текст - Строка - Текст сообщения
...
// 
// Возвращаемое значение:
//  Структура - Информация о отправленном сообщении:
//  * ИдентификаторЧата - Строка, Неопределено - ID чата
//  * ИдентификаторСообщения - Строка, Неопределено - ID сообщения
//  * Успешно - Булево - Флаг успешной отправки
Функция ОтправитьСообщениеСВозвратомId(Знач Получатель, Знач Текст, Знач ДополнительныеСвойства = Неопределено) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ИдентификаторЧата", Неопределено);
	Результат.Вставить("ИдентификаторСообщения", Неопределено);
	...
	Попытка
		...
		Если Ответ.КодСостояния = 200 Тогда
			// 👇 ПАРСИМ ОТВЕТ TELEGRAM API
			ТелоОтвета = СериализацияДанных.СтрокуJSONВОбъект(Ответ.ПолучитьТелоКакСтроку());
			
			Если ТелоОтвета.Свойство("result") И ТелоОтвета.result.Свойство("message_id") Тогда
				Результат.ИдентификаторЧата = ИдентификаторПолучателя;
				Результат.ИдентификаторСообщения = ТелоОтвета.result.message_id;
				Результат.Успешно = Истина;
			КонецЕсли;
		КонецЕсли;
		
	Исключение
		// Ошибка отправки — результат остаётся с Успешно = Ложь
		ЗаписьЖурналаРегистрации(
			"Telegram_SendMessage_Error",
			УровеньЖурналаРегистрации.Ошибка,
			,
			СтрШаблон("chat_id=%1, Ошибка: %2", ИдентификаторПолучателя, ОписаниеОшибки()));
	КонецПопытки;
...
Процедура УдалитьСообщение(ИдентификаторЧата, ИдентификаторСообщения) Экспорт
	
	Метод = ИнтеграцияТелеграмСлужебный.МетодУдалитьСообщение();
	ОписаниеЗапроса = ИнтеграцияТелеграмСлужебный.ОписаниеЗапросаТелеграм(Метод);
